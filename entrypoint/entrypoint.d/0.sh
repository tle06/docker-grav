...
VAR_PHP_FILE_UPLOAD="${PHP_FILE_UPLOAD:-off}"
VAR_PHP_ALLOW_URL="${PHP_ALLOW_URL:-off}"
...
sed -i -e "s/file_uploads\s*=\s*off/file_uploads = $VAR_PHP_FILE_UPLOAD/g" /etc/php/7.2/apache2/php.ini
sed -i -e "s/allow_url_fopen\s*=\s*off/allow_url_fopen = $VAR_PHP_ALLOW_URL/g" /etc/php/7.2/apache2/php.ini
